...
The damage increases with your Mindpower.]]):tformat(damDesc(self, DamageType.MIND, damage), hateGain, math.min(jumpCount, t.getDuration(self, t)), jumpRange, jumpChance, Desc.vs(Desc.mp, Desc.ms))
	end,
}
...
The damage increases with your Mindpower.]]):tformat(duration, damDesc(self, DamageType.MIND, minDamage), damDesc(self, DamageType.MIND, maxDamage), maxDamage * (duration + 1) / 2, Desc.vs(Desc.mp, Desc.ms))
	end,
}
...
newTalent{
	...
	info = function(self, t)
		...
		return ([[Your hateful will splinters into the minds of those you torture, breaking them down. Each time you inflict Mind damage, the victim has a %0.1f%% chance of going mad for 3 turns %s. The madness will lower the victim's Mind resistance by %0.1f%% and cause them to become confused (50%% power), slowed (30%% power), or stunned for the duration.]]):tformat(chance, Desc.vs(Desc.mp, Desc.ms), -mindResistChange)
	end,
}
