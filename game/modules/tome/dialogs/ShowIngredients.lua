...
function _M:generateList()
	...
	for id, nb in pairs(self.party.ingredients) do
		...
		list[#list+1] = { dname=d.name, name=d.display_entity:getDisplayString(true):add(_t(d.name, "ingredient name")), desc=util.getval(d.desc), cat=_t(d.type, "ingredient type"), nb=nb==-1 and "inf" or tostring(nb) }
		i = i + 1
	end
	-- Add known artifacts
